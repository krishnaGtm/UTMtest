ALTER TABLE S2SCapacitySlot
ADD CapacitySlotName NVARCHAR(50)

GO

/****** Object:  StoredProcedure [dbo].[PR_Import_s2s_Material]    Script Date: 5/17/2019 11:21:32 AM ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[PR_Import_s2s_Material]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[PR_Import_s2s_Material]
GO
/****** Object:  UserDefinedTableType [dbo].[TVPCapacityS2s]    Script Date: 5/17/2019 11:21:32 AM ******/
IF  EXISTS (SELECT * FROM sys.types st JOIN sys.schemas ss ON st.schema_id = ss.schema_id WHERE st.name = N'TVPCapacityS2s' AND ss.name = N'dbo')
DROP TYPE [dbo].[TVPCapacityS2s]
GO
/****** Object:  UserDefinedTableType [dbo].[TVPCapacityS2s]    Script Date: 5/17/2019 11:21:32 AM ******/
CREATE TYPE [dbo].[TVPCapacityS2s] AS TABLE(
	[CapacitySlotID] [int] NULL,
	[CapacitySlotName] [nvarchar](max) NULL,
	[MaxPlants] [int] NULL,
