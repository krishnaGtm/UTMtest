	...
	(
		SELECT 
			...
			LEFT JOIN CnTInfo I ON I.RowID = R.RowID
			 ...
			 SELECT CTE.UtmID,CTE.[Crop Code],CTE.GID,CTE.Gen,CTE.[Plant name],CTE.PHID,CTE.Process,CTE.[LAB Location],CTE.[Start Material],CTE.[Type],CTE.Transplant,CTE.Requested,CTE.Net,CTE.[Requested Date],CTE.Delivered,CTE.[Notes/Remarks],CTE.Determinations--, Count_CTE.MaxMarker 
			 FROM CTE--, COUNT_CTE
	ORDER BY CTE.UtmID
END