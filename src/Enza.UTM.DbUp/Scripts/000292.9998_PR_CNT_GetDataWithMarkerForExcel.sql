...
KRIAHNA GAUTAM			2020-FEB-04			#9998 Total Determination column removed (because default marker starting from Marker01 to Marker20 must be present in excel)
=================Example===============
	...
	(
		SELECT 
			...
			LEFT JOIN CnTInfo I ON I.RowID = R.RowID
			 ...
			 SELECT CTE.UtmID,CTE.[Crop Code],CTE.GID,CTE.Gen,CTE.[Plant name],CTE.PHID, CTE.[Donor Number], CTE.Process,CTE.[LAB Location],CTE.[Start Material],CTE.[Type],CTE.Transplant,CTE.Requested,CTE.Net,CTE.[Requested Date],CTE.Delivered,CTE.[Notes/Remarks],CTE.Determinations--, Count_CTE.MaxMarker 
			 FROM CTE--, COUNT_CTE
	ORDER BY CTE.UtmID
END