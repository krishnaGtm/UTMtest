...
AS BEGIN
	...
	IF(ISNULL(@Columns,'') = '') BEGIN
		...
		(
			SELECT M.MaterialID, T1.RowID, T1.RowNr, T1.Selected, M.MaterialKey,D.DH0Net,D.Requested,D.Transplant,D.ToBeSown, ' + @Columns3 + N'
			FROM 
			(
	...
	ELSE BEGIN
		...
		SELECT MaterialID, MaterialKey, D_Selected = Selected, DH0Net, Requested, Transplant, ToBeSown, ' + @Columns3 + N', CTE_COUNT.TotalRows 
		FROM CTE, CTE_COUNT
		ORDER BY RowNr
		OFFSET @Offset ROWS
		...
		VALUES('DH0Net', 2, 'INT', 3),('Requested', 2, 'INT', 4),('Transplant', 2, 'INT', 5),('ToBeSown', 2, 'INT', 6);

		INSERT INTO @TblColumns(Traitid,ColumnLabel, ColumnType, DataType, ColumnNr)
		VALUES('D_Selected','Selected',2,'BIT',2);
