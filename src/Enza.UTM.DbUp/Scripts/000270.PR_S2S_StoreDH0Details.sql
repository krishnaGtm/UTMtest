...
(
	...
	BEGIN TRY
		BEGIN TRANSACTION;
				...
				SET @IDs = NULL;
				SELECT DISTINCT
				   @IDs = COALESCE(@IDs + ',', '') + CAST(T.MaterialID AS NVARCHAR(20))
				FROM @Tbl T
				LEFT JOIN 
				(
					SELECT T.TestID, M.MaterialID FROM Test T 
					JOIN [File] F ON F.FileID = T.FileID
					JOIN [Row] R ON R.FileID = T.FileID
					JOIN Material M ON M.MaterialKey = R.MaterialKey
				) T1 ON T1.TestID = T.TestID AND T1.MaterialID = T.MaterialID
				WHERE T1.MaterialID IS NULL;
