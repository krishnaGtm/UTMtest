...
(
	...
	SET @Query = N'SELECT --ROW_NUMBER() OVER(ORDER BY V1.MaterialKey ASC) AS RowNr, 
		V1.*, '  + @MarkerColumnIDs + '  FROM
	(
		SELECT RowID, ' + @Columns + N' 
		FROM 
		(
	...
	SELECT 'RowID' AS ColumnID, 'RowID' AS ColumnLabel
	UNION ALL
	SELECT 
