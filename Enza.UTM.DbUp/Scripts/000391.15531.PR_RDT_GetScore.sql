...
BEGIN
	...
	BEGIN
		...
		LEFT JOIN RDTTraitDetResult TDR ON 
					...
					AND (
							(TR.SusceptibilityPercent BETWEEN TDR.MinPercent AND TDR.MaxPercent)
							OR
							(ISNULL(TR.Score,'') <> '' AND (ISNULL(TDR.MinPercent,0) = 0 OR ISNULL(TDR.MaxPercent,0) = 0))
						)
					AND CASE WHEN ISNULL(TM.MaterialStatus,'') = ISNULL(TDR.MaterialStatus,'') THEN 1
							WHEN ISNULL(TDR.MaterialStatus,'') = '' THEN 1
							ELSE 0
							END = 1
		WHERE T.TestID = @TestID AND TR.ResultStatus IN (100,200)
		order by MaterialID, TDR.MaterialStatus DESC
