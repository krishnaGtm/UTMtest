...
EXEC [PR_GetDataWithMarkers] 4556, 1, 1000, '[plant name] LIKE ''%401%'''
*/
...
(
	...
	SELECT @FileID = F.FileID, @FileName = T.TestName, @Crop = CropCode, @Source = T.RequestingSystem, @StatusCode = StatusCode, @Rearrange = ISNULL(T.RearrangePlateFilling,1),@ImportLevel = ImportLevel,@TestTypeID = T.TestTypeID
	FROM [File] F
	JOIN Test T ON T.FileID = F.FileID 
	WHERE T.TestID = @TestID

	SELECT @TotalRowsWithoutFilter = CAST(COUNT(W.WellID) AS VARCHAR(10)) FROM TestMaterialDeterminationWell TMDW
				...
				BEGIN /*BEGIN - Add material to Plate region*/
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
								EXEC PR_ThrowError 'More plates are needed than requested plates from LIMS. Remove some materials from plate.';
								RETURN;
							END
	...
	(
		...
		V1.WellTypeID, V1.Fixed, Total = '''+ @TotalRowsWithoutFilter +''',
		' + @Columns3 + N'
		FROM 
		(
			SELECT
	...
	SELECT CTE.MaterialID,CTE.WellTypeID, CTE.WellID, CTE.Fixed, CTE.Plate, CTE.Well,  
	' + @Columns3 + N', CTE_COUNT.TotalRows, CTE.Total
	FROM CTE, CTE_COUNT
	ORDER BY WellID
	OFFSET @Offset ROWS
