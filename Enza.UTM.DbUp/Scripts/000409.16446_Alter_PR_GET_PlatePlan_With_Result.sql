IF EXISTS (SELECT * FROM SYS.TYPES WHERE NAME = 'TVP_Test_Wells')
	DROP TYPE TVP_Test_Wells

GO

CREATE TYPE TVP_Test_Wells AS TABLE (TestID INT, PlateID INT, PlateName NVARCHAR(150), WellID INT, Position NVARCHAR(5), RowID INT);
GO
...
AS BEGIN
	...
	SELECT @FileID = FileID,@ImportLabel = ImportLevel, @IsBTR = BTR, @TestType = TestTypeID FROM Test where TestID = @TestID;
	...
	SELECT 
		@DeterminationsName = COALESCE(@DeterminationsName +',','') + 'ISNULL(' + QUOTENAME(DeterminationID) + ',''EMPTY'') AS ' + QUOTENAME(DeterminationName),
	...
	SET @Query = N'
				SELECT 
					T0.PlateName,
					Well = T0.Position,
					' +QuoteName(@ColumnName)+ ' = ISNULL(T1.'+@ColID+',''EMPTY'')
