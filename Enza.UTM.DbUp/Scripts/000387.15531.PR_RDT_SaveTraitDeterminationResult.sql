...
(
	...
	JOIN RDTTraitDetResult R ON ISNULL(R.DetResult,'') = ISNULL(T.DetResult,'') AND ISNULL(T.MappingCol,'') = ISNULL(R.MappingCol,'') AND ISNULL(T.MinPercent,0) = ISNULL(R.MinPercent,0) AND ISNULL(T.MaxPercent,0) = ISNULL(R.MaxPercent,0) AND ISNULL(T.MaterialStatus,'') = ISNULL(R.MaterialStatus,'') AND ISNULL(T.DetResult,'') = ISNULL(R.DetResult,'') AND ISNULL(T.RelationID,0) = ISNULL(R.RelationID,0)
	WHERE T.Action = 'I')
	BEGIN		
