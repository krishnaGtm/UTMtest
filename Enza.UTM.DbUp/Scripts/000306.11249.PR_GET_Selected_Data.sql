	...
	BEGIN
		SET @Query = N' ;WITH CTE AS 
						(
						  SELECT * FROM
						  (
							SELECT M.MaterialID, R.FileID, R.[RowNr], D_Selected = R.Selected, R.MaterialKey, R.NrOfSamples, ' + @Columns2 + '
							FROM [ROW] R 
	...
	BEGIN
		SET @Query = N' ;WITH CTE AS 
						(
							SELECT * FROM
							(
							    SELECT M.MaterialID, R.FileID, R.[RowNr], D_Selected = R.Selected, R.MaterialKey, ' + @Columns2 + ' 
							    FROM [ROW] R 
							    JOIN Material M ON M.Materialkey = R.MaterialKey 	
							    LEFT JOIN 
							    (
						...
						SELECT CTE.MaterialID, CTE.MaterialKey, CTE.D_Selected , '+ @Columns2 + ', Count_CTE.[TotalRows] FROM CTE, COUNT_CTE
						ORDER BY CTE.[RowNr]
						OFFSET ' + CAST(@Offset AS NVARCHAR) + ' ROWS
						FETCH NEXT ' + CAST (@PageSize AS NVARCHAR) + ' ROWS ONLY';
	END
