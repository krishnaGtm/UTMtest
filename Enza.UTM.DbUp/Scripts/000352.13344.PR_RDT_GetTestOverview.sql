...
DROP PROCEDURE IF EXISTS [dbo].[PR_RDT_GetTestOverview]
GO
...
CREATE PROCEDURE [dbo].[PR_RDT_GetTestOverview]
(
    @Active	BIT  = NULL,
	@Crops NVARCHAR(MAX),
	@Filter NVARCHAR(MAX),
	...
	SET @Query =	N';WITH CTE AS 
					(
						...
						(
							...
							WHERE T.TestTypeID = 8 AND F.CropCode IN  ('+@CropCodes +') AND T.CreationDate >= DATEADD(YEAR,-1,GetDate()) AND T.StatusCode >= 200
						) T1 '+@Filter+'
