...
Krishna Gautam		2020-09-16			#15683: add site location.	
	...
	SET @Query =	N';WITH CTE AS 
					(
						...
						(
							...
							LEFT JOIN SiteLocation SL ON SL.SiteID = T.SiteID
							LEFT JOIN
							(
								SELECT 
									TestID, 
									ExpectedDate = MAX(ExpectedDate),
									UsedTests = SUM(ISNULL(UsedTests,0))
								FROM
								(
									SELECT 
										TestID, 
										MaterialID,
										ExpectedDate = MAX(ExpectedDate),
										UsedTests = COUNT(DISTINCT DeterminationID)
