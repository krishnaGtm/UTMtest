...
CREATE PROCEDURE PR_RDT_GetMaterialToPrint
(
	@TestID INT,
...
AS BEGIN
	...
	BEGIN
		...
		JOIN string_split(@MaterialStatus,',') MS ON MS.[Value] = TM.MaterialStatus --join material status here
	...
	ELSE IF EXISTS(SELECT TOP 1 * FROM @TVP_TMD)
	BEGIN
