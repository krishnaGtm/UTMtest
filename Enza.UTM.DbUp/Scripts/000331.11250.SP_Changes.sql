...
INSERT INTO WellTypePosition(WellTypeID,TestTypeID,PositionOnPlate)
VALUES
(2,2,'B01'),
(2,2,'D01'),
(2,2,'F01'),
...
AS BEGIN
	...
	WHILE(@Count < @TotalPlates) BEGIN
		...
		IF(ISNULL(@ExcludeControlPosition,0) = 1)
		BEGIN
			INSERT INTO @Table(Position,PlateID,PlateName,MaterialKey,BgColor,FgColor)
...
BEGIN  
    ...
    SELECT   
	 ...
	 T.Isolated,  
	 ST1.SlotID,  
	 WellsPerPlate = CASE WHEN ISNULL(T.ExcludeControlPosition,0) = 1 THEN  @TotalWells - ISNULL(T3.Blocked,0) ELSE @TotalWells END,  
